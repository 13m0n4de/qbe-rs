...
- `From<u64>` implementation for `Value`, allowing `Value::from(42)` or `42.into()` as shorthand for `Value::Const(42)` ([#57](https://github.com/garritfra/qbe-rs/pull/57))
- `categories` field in Cargo.toml metadata ([#57](https://github.com/garritfra/qbe-rs/pull/57))

### Improved

- Panic messages in `Display` impls now include descriptive context (e.g. `"cannot store to an aggregate type"` instead of `unimplemented!()`) ([#57](https://github.com/garritfra/qbe-rs/pull/57))
- `Function::add_instr` and `Function::assign_instr` document their panic conditions ([#57](https://github.com/garritfra/qbe-rs/pull/57))
